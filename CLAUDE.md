...
成果物（.md・.canvas・HTML・PDF等）のヘッダーには、日付に加えて **時刻（HH:MM）** を必ず付記すること。

- `.md` ファイル: `作成日: YYYY-MM-DD HH:MM`
- `.canvas` タイトルノード: `／ YYYY-MM-DD HH:MM`
- HTML / PDF: ヘッダー部分に `YYYY-MM-DD HH:MM` 形式で記載
- 時刻は `date "+%Y-%m-%d %H:%M"` コマンドで取得する（推測・固定値は禁止）

**Why:** 同じ日に複数の成果物が出る場合、日付だけでは最新版を識別できない。
