...
## 設定の一元管理（itokenchiku-tools）

hook・settings.json の変更は itokenchiku-tools/templates/ で一元管理する。
個別リポの .claude/settings.json を直接編集してはいけない。
変更は itokenchiku-tools を編集 → sync-hooks.sh で全リポに反映すること。
